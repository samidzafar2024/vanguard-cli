...
# WayPoint: Update Framework

Pull the latest agent framework bundle from the WayPoint platform and apply files non-destructively, respecting each file's write strategy.

## Instructions

1. Fetch the latest bundle from the WayPoint API:
   - Call `GET /api/agent-framework/bundle?project_id=<PROJECT_ID>` (find project_id in `.waypoint/config.yaml`)
   - Or run `waypoint bundle` if using the CLI
