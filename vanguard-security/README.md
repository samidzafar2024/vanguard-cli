>[!NOTE]
> **[📢 Sunsetting Router Mode (claude-code-router)`. →](https://github.com/CopointAI/vanguard/discussions/301)**

<div align="center">

<img src="./assets/github-banner.png" alt="Vanguard — AI Pentester for Web Applications and APIs" width="100%">

# Vanguard — AI Pentester by CopointAI

<a href="https://trendshift.io/repositories/15604" target="_blank"><img src="https://trendshift.io/api/badge/repositories/15604" alt="CopointAI%2Fvanguard | Trendshift" style="width: 250px; height: 55px;" width="250" height="55"/></a>

Vanguard is an autonomous, white-box AI pentester for web applications and APIs. <br />
...
<a href="https://copointai.io/"><img src="./assets/CopointAI_Button.png" height="40" alt="Visit CopointAI.io"></a>
...
Vanguard is an AI pentester developed by [CopointAI](https://copointai.io). It performs white-box security testing of web applications and their underlying APIs by combining source code analysis with live exploitation.

Vanguard analyzes your web application's source code to identify potential attack vectors, then uses browser automation and command-line tools to execute real exploits (injection attacks, authentication bypass, SSRF, XSS) against the running application and its APIs. Only vulnerabilities with a working proof-of-concept are included in the final report.

**Why Vanguard Exists**

Thanks to tools like Claude Code and Cursor, your team ships code non-stop. But your penetration test? That happens once a year. This creates a *massive* security gap. For the other 364 days, you could be unknowingly shipping vulnerabilities to production.

Vanguard closes that gap by providing on-demand, automated penetration testing that can run against every build or release.

## Vanguard in Action

Vanguard identified 20+ vulnerabilities in OWASP Juice Shop, including authentication bypass and database exfiltration. [Full report →](sample-reports/vanguard-report-juice-shop.md)

![Demo](assets/vanguard-action.gif)

## Features

- **Fully Autonomous Operation**: A single command launches the full pentest. Vanguard handles 2FA/TOTP logins (including SSO), browser navigation, exploitation, and report generation without manual intervention.
...
Vanguard is developed by [CopointAI](https://copointai.io) and available in two editions:

| Edition | License | Best For |
|---------|---------|----------|
| **Vanguard Lite** | AGPL-3.0 | Local testing of your own applications. |
| **Vanguard Pro** | Commercial | Organizations needing a single AppSec platform (SAST, SCA, secrets, business logic testing, autonomous pentesting) with CI/CD integration and self-hosted deployment. |

> **This repository contains Vanguard Lite,** the core autonomous AI pentesting framework. **Vanguard Pro** is CopointAI's all-in-one AppSec platform, combining SAST, SCA, secrets scanning, business logic security testing, and autonomous AI pentesting in a single correlated workflow. Every finding is validated with a working proof-of-concept exploit.

> [!IMPORTANT]
> **White-box only.** Vanguard Lite is designed for **white-box (source-available)** application security testing.  
> It expects access to your application's source code and repository layout.

### Vanguard Pro: Architecture Overview

Vanguard Pro is an all-in-one application security platform that replaces the need to stitch together separate SAST, SCA, secrets scanning, and pentesting tools. It operates as a two-stage pipeline: agentic static analysis of the codebase, followed by autonomous AI penetration testing. Findings from both stages are cross-referenced and correlated, so every reported vulnerability has a working proof-of-concept exploit and a precise source code location.

**Stage 1: Agentic Static Analysis**

Vanguard Pro transforms the codebase into a Code Property Graph (CPG) combining the AST, control flow graph, and program dependence graph. It then runs five analysis capabilities:
...
The same multi-agent pentest pipeline as Vanguard Lite (reconnaissance, parallel vulnerability analysis, parallel exploitation, reporting), enhanced with static findings injected into the exploitation queue. Static findings are mapped to Vanguard's five attack domains (Injection, XSS, SSRF, Auth, Authz), and exploit agents attempt real proof-of-concept attacks against the running application for each finding.
...
Vanguard Pro supports a self-hosted runner model (similar to GitHub Actions self-hosted runners). The data plane, which handles code access and all LLM API calls, runs entirely within the customer's infrastructure using the customer's own API keys. Source code never leaves the customer's network. The CopointAI control plane handles job orchestration, scan scheduling, and the reporting UI, receiving only aggregate findings.

| Capability | Vanguard Lite | Vanguard Pro (All-in-One AppSec) |
| --- | --- | --- |
...
[Full technical details →](./VANGUARD-PRO.md)

## Table of Contents

- [What is Vanguard?](#what-is-vanguard)
- [Vanguard in Action](#vanguard-in-action)
- [Features](#features)
...
> [!NOTE]
> Docker is still required to use the `npx` workflow. Under the hood, the CLI pulls and runs a prebuilt Vanguard worker image from Docker Hub, which is approximately 1 GB and contains Vanguard plus all required dependencies. Vanguard mounts the target repository as read-only inside the worker container to protect against accidental modifications during analysis. Run Vanguard via `npx @copointai/vanguard` for the latest released version, or pull the latest `main` if building from source.

### Quick Start (Recommended: npx)

> [!WARNING]
> **Please read the [Disclaimers](#disclaimers) before running Vanguard.** Vanguard is **not** a passive scanner — it actively executes exploits against the target. You must have **explicit, written authorization** from the system owner.

```bash
# 1. Configure credentials (interactive wizard — one-time setup)
npx @copointai/vanguard setup

# Or export env vars directly
export ANTHROPIC_API_KEY=your-api-key

# 2. Run a pentest
npx @copointai/vanguard start -u https://your-app.com -r /path/to/your-repo
```

Vanguard will pull the worker image from Docker Hub, start the infrastructure, and launch an ephemeral worker container for the scan.

### Clone and Build

Use this if you want to run Vanguard from a local clone, modify Vanguard itself, or keep the worker image built locally.

```bash
# 1. Clone Vanguard
git clone https://github.com/CopointAI/vanguard.git
cd vanguard
...
# 4. Run a pentest
./vanguard start -u https://your-app.com -r /path/to/your-repo
```

Vanguard will build the worker image locally, start the infrastructure, and launch an ephemeral worker container for the scan.

### Prepare Your Repository

Vanguard can scan any repository on your machine. Pass an absolute or relative path with `-r`.

Examples:

```bash
npx @copointai/vanguard start -u https://example.com -r /path/to/repo
```
...
```bash
./vanguard start -u https://example.com -r ./relative/path
```
...
```bash
npx @copointai/vanguard logs <workspace>
npx @copointai/vanguard status
```
...
```bash
npx @copointai/vanguard stop
npx @copointai/vanguard stop --clean
npx @copointai/vanguard uninstall
```
...
npx @copointai/vanguard start -u https://example.com -r /path/to/repo -c /path/to/my-config.yaml

# Custom output directory
npx @copointai/vanguard start -u https://example.com -r /path/to/repo -o ./my-reports

# Named workspace
npx @copointai/vanguard start -u https://example.com -r /path/to/repo -w q1-audit

# List all workspaces
npx @copointai/vanguard workspaces
```
...
```bash
# Basic pentest
./vanguard start -u https://example.com -r /path/to/repo

# With a configuration file
./vanguard start -u https://example.com -r /path/to/repo -c /path/to/my-config.yaml

# Custom output directory
./vanguard start -u https://example.com -r /path/to/repo -o ./my-reports

# Named workspace
./vanguard start -u https://example.com -r /path/to/repo -w q1-audit

# List all workspaces
./vanguard workspaces

# Rebuild worker image
./vanguard build --no-cache
```
...
Vanguard supports **workspaces** that allow you to resume interrupted or failed runs without re-running completed agents.

**How it works:**

- Every run creates a workspace (auto-named by default, for example `example-com_vanguard-1771007534808`)
- Workspaces are stored in `./workspaces/` (local mode) or `~/.vanguard/workspaces/` (npx mode)
- Use `-w <name>` to give your run a custom name for easier reference
- To resume any run, pass its workspace name via `-w` — Vanguard detects which agents completed successfully and picks up where it left off
- Each agent's progress is checkpointed via git commits, so resumed runs start from a clean, validated state

```bash
# Start with a named workspace
npx @copointai/vanguard start -u https://example.com -r /path/to/repo -w my-audit

# Resume the same workspace (skips completed agents)
npx @copointai/vanguard start -u https://example.com -r /path/to/repo -w my-audit

# Resume an auto-named workspace from a previous run
npx @copointai/vanguard start -u https://example.com -r /path/to/repo -w example-com_vanguard-1771007534808
...
```bash
./vanguard start -u https://example.com -r /path/to/repo -w my-audit
./vanguard start -u https://example.com -r /path/to/repo -w my-audit
./vanguard start -u https://example.com -r /path/to/repo -w example-com_vanguard-1771007534808
./vanguard workspaces
```
...
> [!NOTE]
> The `URL` must match the original workspace URL when resuming. Vanguard will reject mismatched URLs to prevent cross-target contamination.
...
1. **Environment variables** - `export ANTHROPIC_API_KEY=...`
2. **`~/.vanguard/config.toml`** - created by `npx @copointai/vanguard setup`
...
```bash
npx @copointai/vanguard start -u https://example.com -r /path/to/repo -c ./my-app-config.yaml
```
...
```bash
./vanguard start -u https://example.com -r /path/to/repo -c ./my-app-config.yaml
```
...
Vanguard also supports [Amazon Bedrock](https://aws.amazon.com/bedrock/) instead of using an Anthropic API key.

#### Quick Setup

Run `npx @copointai/vanguard setup` and select **AWS Bedrock**. The wizard will prompt for your region, bearer token, and model IDs.
...
Vanguard uses three model tiers: **small** (`claude-haiku-4-5-20251001`) for summarization, **medium** (`claude-sonnet-4-6`) for security analysis, and **large** (`claude-opus-4-6`) for deep reasoning. Set `ANTHROPIC_SMALL_MODEL`, `ANTHROPIC_MEDIUM_MODEL`, and `ANTHROPIC_LARGE_MODEL` to the Bedrock model IDs for your region.

### Google Vertex AI

Vanguard also supports [Google Vertex AI](https://cloud.google.com/vertex-ai) instead of using an Anthropic API key.
...
Run `npx @copointai/vanguard setup` and select **Google Vertex AI**. The wizard will prompt for your region, project ID, service account key file path, and model IDs. The key file is securely copied to `~/.vanguard/google-sa-key.json`.
...
Vanguard supports pointing the SDK at any Anthropic-compatible endpoint via `ANTHROPIC_BASE_URL`. For users who need proxy-based routing, the supported path is to use an LLM proxy such as [LiteLLM](https://github.com/BerriAI/litellm) configured to expose an Anthropic-compatible endpoint.

> [!IMPORTANT]
> **Only Claude models are officially supported.** Vanguard's evaluations, internal testing, and agent harness are all optimized for Claude. Smaller or alternative models — including non-Claude models routed through a proxy — may not reliably follow Vanguard's instructions or tool-use constraints, and are not officially supported. Use them at your own risk; results may be incomplete, inaccurate, or unstable.
>
...
Run `npx @copointai/vanguard setup` and select **Custom Base URL**. The wizard will prompt for your endpoint URL, auth token, and optionally let you override the default model tiers.
...
Vanguard on Windows is only supported via **WSL2**. Native Windows (including Git Bash) is not supported.
...
**Step 3: Run Vanguard inside WSL** using either flow.
...
```bash
npx @copointai/vanguard start -u http://host.docker.internal:3000 -r /path/to/repo
```
...
```bash
./vanguard start -u http://host.docker.internal:3000 -r /path/to/repo
```
...
All results are saved to the workspaces directory: `./workspaces/` (local mode) or `~/.vanguard/workspaces/` (npx mode). Use `-o <path>` to copy deliverables to a custom output directory after the run completes.
...
[View Complete Report →](sample-reports/vanguard-report-juice-shop.md)
...
[View Complete Report →](sample-reports/vanguard-report-capital-api.md)
...
[View Complete Report →](sample-reports/vanguard-report-crapi.md)

---

## Benchmark

Vanguard Lite scored **96.15% (100/104 exploits)** on a hint-free, source-aware variant of the XBOW security benchmark.

**[Full results with detailed agent logs and per-challenge pentest reports →](https://github.com/CopointAI/xbow-validation-benchmarks/blob/main/xben-benchmark-results/)**

---

## Architecture

Vanguard uses a multi-agent architecture that combines white-box source code analysis with dynamic exploitation across five phases:
...
Builds a comprehensive attack surface map from the pre-recon findings. Vanguard performs live application exploration via browser automation to correlate code-level insights with real-world behavior, producing a detailed map of all entry points, API endpoints, and authentication mechanisms.
...
For detailed information about Vanguard's security testing coverage and development roadmap, see our [Coverage and Roadmap](./COVERAGE.md) documentation.

## Disclaimers

### Important Usage Guidelines & Disclaimers

Please review the following guidelines carefully before using Vanguard (Lite). As a user, you are responsible for your actions and assume all liability.
...
> [!WARNING]
> **DO NOT run Vanguard on production environments.**
>
...
> - **For maximum security and isolation, run Vanguard inside a virtual machine (VM).** This confines any side effects from exploitation — including unexpected outbound traffic, file writes from agent tooling, or interactions with local services — to a disposable environment.

#### **2. Legal & Ethical Use**

Vanguard is designed for legitimate security auditing purposes only.

> [!CAUTION]
> **You must have explicit, written authorization** from the owner of the target system before running Vanguard.
>
> Unauthorized scanning and exploitation of systems you do not own is illegal and can be prosecuted under laws such as the Computer Fraud and Abuse Act (CFAA). CopointAI is not responsible for any misuse of Vanguard.
...
- **Model Support**: Vanguard is officially supported only with **Claude models**. Our evaluations, internal testing, and agent harness are all optimized for Claude. Smaller or alternative models — including non-Claude models routed through a proxy — may not reliably follow Vanguard's instructions or tool-use constraints, and are not officially supported.
- **Comprehensiveness**: The analysis in Vanguard Lite may not be exhaustive due to the inherent limitations of LLM context windows. For a more comprehensive, graph-based analysis of your entire codebase, **Vanguard Pro** leverages its advanced data flow analysis engine to ensure deeper and more thorough coverage.

#### **4. Scope of Analysis**

- **Targeted Vulnerabilities**: The current version of Vanguard Lite specifically targets the following classes of *exploitable* vulnerabilities:
  - Broken Authentication & Authorization
  - Injection
  - Cross-Site Scripting (XSS)
  - Server-Side Request Forgery (SSRF)
- **What Vanguard Lite Does Not Cover**: This list is not exhaustive of all potential security risks. Vanguard Lite's "proof-by-exploitation" model means it will not report on issues it cannot actively exploit, such as vulnerable third-party libraries or insecure configurations. These types of deep static-analysis findings are a core focus of the advanced analysis engine in **Vanguard Pro**.
...
Windows Defender may flag files in `xben-benchmark-results/` or `deliverables/` as malware. These are false positives caused by exploit code in the reports. Add an exclusion for the Vanguard directory in Windows Defender, or use Docker/WSL2.

#### **7. Security Considerations**

Vanguard Lite is designed for scanning repositories and applications you own or have explicit permission to test. Do not point it at untrusted or adversarial codebases. Like any AI-powered tool that reads source code, Vanguard Lite is susceptible to prompt injection from content in the scanned repository.


## License

Vanguard Lite is released under the [GNU Affero General Public License v3.0 (AGPL-3.0)](LICENSE).

Vanguard is open source (AGPL v3). This license allows you to:
- Use it freely for all internal security testing.
- Modify the code privately for internal use without sharing your changes.

The AGPL's sharing requirements primarily apply to organizations offering Vanguard as a public or managed service (such as a SaaS platform). In those specific cases, any modifications made to the core software must be open-sourced.
...
→ [Book a slot](https://cal.com/george-flores-copointai/vanguard-community-office-hours)

[Join our Discord](https://discord.gg/cmctpMBXwE) to ask questions, share feedback, and connect with other Vanguard users.
...
- **Report bugs** via [GitHub Issues](https://github.com/CopointAI/vanguard/issues)
- **Suggest features** in [Discussions](https://github.com/CopointAI/vanguard/discussions)

### Stay Connected

- **Twitter**: [@CopointAI](https://twitter.com/CopointAI)
- **LinkedIn**: [CopointAI](https://linkedin.com/company/copointai)
- **Website**: [copointai.io](https://copointai.io)
...
Vanguard Pro is CopointAI's all-in-one AppSec platform. For organizations that need unified SAST, SCA, and autonomous pentesting with static-dynamic correlation, CI/CD integration, or self-hosted deployment, see the [Vanguard Pro technical overview](./VANGUARD-PRO.md).

<p align="center">
  <a href="https://cal.com/team/copointai/vanguard-pro" target="_blank">
    <img src="./assets/Demo_Button.png" height="40" alt="Vanguard Pro Inquiry">
  </a>
</p>

**Email**: [vanguard@copointai.io](mailto:vanguard@copointai.io)

---

<p align="center">
  <b>Built by <a href="https://copointai.io">CopointAI</a></b>
</p>
