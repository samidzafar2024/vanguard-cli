/**
 * `vanguard start` command — launch a pentest scan.
 *
 * Handles both local mode (local build, ./workspaces/, mounted prompts)
 * and npx mode (Docker Hub pull, ~/.vanguard/).
 */
...
export async function start(args: StartArgs): Promise<void> {
  ...
  const taskQueue = `vanguard-${suffix}`;
  const containerName = `vanguard-worker-${suffix}`;

  // 7. Generate workspace name if not provided
  const workspace =
    args.workspace ?? `${new URL(args.url).hostname.replace(/[^a-zA-Z0-9-]/g, '-')}_vanguard-${Date.now()}`;
  ...
  const vanguardDir = path.join(repo.hostPath, '.vanguard');
  for (const dir of ['deliverables', 'scratchpad', '.playwright-cli']) {
    fs.mkdirSync(path.join(vanguardDir, dir), { recursive: true });
  }
  ...
  const proc = spawnWorker({
    ...
    ...(args.reconUrl && { reconUrl: args.reconUrl }),
  });
...
function printInfo(
  ...
  const logsCmd = isLocal() ? `./vanguard logs ${workspace}` : `npx @copointai/vanguard logs ${workspace}`;
  const reportsPath = path.join(workspacesDir, workspace);

  console.log(`  Target:     ${args.url}`);
  if (args.reconUrl) {
    console.log(`  Recon URL:  ${args.reconUrl}`);
  }
