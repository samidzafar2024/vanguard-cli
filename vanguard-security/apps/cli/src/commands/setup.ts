/**
 * `vanguard setup` — interactive TUI wizard for one-time credential configuration.
 *
 * Walks the user through selecting a provider and entering credentials,
 * then persists everything to ~/.vanguard/config.toml with 0o600 permissions.
 */
...
import { saveConfig, type VanguardConfig } from '../config/writer.js';
...
export async function setup(): Promise<void> {
  ...
  const configPath = path.join(VANGUARD_HOME, 'config.toml');
  p.log.success(`Configuration saved to ${configPath}`);
  p.outro('Run `npx @copointai/vanguard start` to begin a scan.');
}

async function setupProvider(provider: Provider): Promise<VanguardConfig> {
  switch (provider) {
    case 'anthropic':
...
async function setupAnthropic(): Promise<VanguardConfig> {
...
async function setupCustomBaseUrl(): Promise<VanguardConfig> {
...
async function setupBedrock(): Promise<VanguardConfig> {
...
async function setupVertex(): Promise<VanguardConfig> {
  ...
  // 3. Copy key to ~/.vanguard/ and lock permissions
  const destPath = path.join(VANGUARD_HOME, 'google-sa-key.json');
  fs.mkdirSync(VANGUARD_HOME, { recursive: true });
