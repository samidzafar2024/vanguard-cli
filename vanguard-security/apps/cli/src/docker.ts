...
const NPX_IMAGE_REPO = 'copointai/vanguard';
const DEV_IMAGE = 'vanguard-worker';
...
export async function ensureInfra(): Promise<void> {
  ...
  console.log('Starting Vanguard infrastructure...');
...
export function ensureImage(version: string): void {
  ...
  if (getMode() === 'local') {
    ...
    try {
      ...
      console.error('Check https://hub.docker.com/r/copointai/vanguard for available tags.');
      process.exit(1);
    }
...
export function spawnWorker(opts: WorkerOptions): ChildProcess {
  ...
  args.push('--name', opts.containerName, '--network', 'vanguard-net');
  ...
  if (os.platform() === 'linux' && process.getuid && process.getgid) {
    args.push('-e', `VANGUARD_HOST_UID=${process.getuid()}`, '-e', `VANGUARD_HOST_GID=${process.getgid()}`);
  }
  ...
  // Writable overlays: shadow .vanguard/ inside the :ro repo with workspace-backed dirs
  const workspacePath = path.join(opts.workspacesDir, opts.workspace);
  args.push('-v', `${path.join(workspacePath, 'deliverables')}:${opts.repo.containerPath}/.vanguard/deliverables`);
  args.push('-v', `${path.join(workspacePath, 'scratchpad')}:${opts.repo.containerPath}/.vanguard/scratchpad`);
  args.push(
    '-v',
    `${path.join(workspacePath, '.playwright-cli')}:${opts.repo.containerPath}/.vanguard/.playwright-cli`,
  );
  ...
  if (opts.reconUrl) {
    args.push('--recon-url', opts.reconUrl);
  }
...
/**
 * Stop all running vanguard-worker-* containers.
 */
export function stopWorkers(): void {
  const workers = runOutput('docker', ['ps', '-q', '--filter', 'name=vanguard-worker-']);
  if (!workers) return;
...
/**
 * Remove old copointai/vanguard images that don't match the current version.
 */
