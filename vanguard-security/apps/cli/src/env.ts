 ...
 * NPX mode: fills gaps from ~/.vanguard/config.toml (no .env).
 */
...
/**
 * Extract Claude Code OAuth token from the macOS Keychain.
 * Silent no-op on Linux/Windows or if the entry is missing.
 */
function readKeychainOAuthToken(): string {
 ...
 * Local mode: loads ./.env via dotenv.
 * NPX mode: fills gaps from ~/.vanguard/config.toml.
 * Exported env vars always take precedence in both modes.
 * Falls back to Claude Code's macOS Keychain entry when no credentials are set.
 */
export function loadEnv(): void {
  ...
  // Auto-inject Claude Code OAuth token from macOS Keychain if no credentials configured.
  // This allows running Vanguard with just a Claude Code login — no separate API key needed.
...
export function buildEnvFlags(): string[] {
  const flags: string[] = ['-e', 'TEMPORAL_ADDRESS=vanguard-temporal:7233'];
...
export function validateCredentials(): CredentialValidation {
  ...
  const hint =
    getMode() === 'local'
      ...
      : `Authentication not configured. Export variables or run 'npx @copointai/vanguard setup'.`;
  return {
    valid: false,
    mode: 'api-key',
    error: hint,
