 ...
 *
 * Priority: process.env > ~/.vanguard/config.toml
...
function loadTOML(): TOMLConfig | null {
  ...
  try {
    ...
    console.error(`\nRun 'npx @copointai/vanguard setup' to reconfigure.\n`);
    process.exit(1);
  }
}
 ...
 * look it up in ~/.vanguard/config.toml and inject it into process.env.
...
export function resolveConfig(): void {
  ...
  if (errors.length > 0) {
    ...
    console.error(`\nRun 'vanguard setup' to reconfigure.\n`);
    process.exit(1);
  }
