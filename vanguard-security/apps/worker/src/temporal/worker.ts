...
/**
 * Combined Temporal worker + client for Vanguard pentest pipeline.
 *
...
async function resolveWorkspace(client: Client, args: CliArgs): Promise<WorkspaceResolution> {
  if (!args.resumeFromWorkspace) {
    ...
    const workflowId = `${hostname}_vanguard-${Date.now()}`;
    return {
      workflowId,
  ...
  // (ends with _vanguard-<digits>), use it directly to avoid double _vanguard- suffixes
  const workflowId = /_vanguard-\d+$/.test(workspace) ? workspace : `${workspace}_vanguard-${Date.now()}`;
