...
async function interpolateVariables(
  ...
  try {
    ...
    let result = template
      ...
      .replace(/{{BLAST_RADIUS_REMAINING_MB}}/g, variables.blastRadiusRemainingMb ?? 'unknown')
      .replace(/{{BRAIN_HINTS}}/g, variables.brainHints ?? '')
      .replace(/{{PREVIOUS_MEMORY}}/g, variables.previousMemory ?? '');
