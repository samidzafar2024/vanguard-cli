...
export class Container {
  ...
  readonly modeDispatcher: ModeDispatcher;
  readonly blastRadiusTracker: BlastRadiusTracker;
  readonly findingsProvider: FindingsProvider;
  readonly checkpointProvider: CheckpointProvider;
  readonly reportOutputProvider: ReportOutputProvider;
  ...
  constructor(deps: ContainerDependencies) {
    ...
    this.modeDispatcher = new ModeDispatcher(deps.config.engagementMode ?? 'validated');
    this.blastRadiusTracker = new BlastRadiusTracker(deps.config.evidenceBudgetMb ?? 10);
    this.agentExecution = new AgentExecutionService(this.configLoader, this.modeDispatcher, this.blastRadiusTracker);
...
const DEFAULT_CONFIG: ContainerConfig = {
  deliverablesSubdir: '.vanguard/deliverables',
  auditDir: './workspaces',
};
