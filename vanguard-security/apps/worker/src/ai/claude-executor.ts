...
declare global {
  var VANGUARD_DISABLE_LOADER: boolean | undefined;
}
...
export async function runClaudePrompt(
  ...
  const progress = createProgressManager(
    ...
    global.VANGUARD_DISABLE_LOADER ?? false,
  );
...
async function processMessageStream(
  ...
  for await (const message of query({ prompt: fullPrompt, options })) {
    ...
    const now = Date.now();
    if (global.VANGUARD_DISABLE_LOADER && now - lastHeartbeat > HEARTBEAT_INTERVAL) {
