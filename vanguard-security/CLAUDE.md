...
Vanguard supports two CLI modes, auto-detected based on the current working directory:

| | **npx** (`npx @copointai/vanguard`) | **Local** (`./vanguard`) |
|---|---|---|
| **Install** | Zero-install via npm | Clone the repo |
| **Image** | Pulled from Docker Hub (`copointai/vanguard:latest`) | Built locally (`vanguard-worker`) |
| **State** | `~/.vanguard/` | Project directory |
| **Credentials** | `~/.vanguard/config.toml` (via `shn setup`) or env vars | `./.env` |
| **Config** | `~/.vanguard/config.toml` (via `shn setup`) | N/A |
| **Prompts** | Bundled in Docker image | Mounted from `./apps/worker/prompts/` (live-editable) |

Mode auto-detection: local mode activates when env var `VANGUARD_LOCAL=1` is set by the `./vanguard` entry point (`apps/cli/src/mode.ts`). Otherwise npx mode.
...
# Run
npx @copointai/vanguard start -u <url> -r /path/to/repo
```
...
# Run
./vanguard start -u <url> -r my-repo
./vanguard start -u <url> -r my-repo -c ./apps/worker/configs/my-config.yaml
./vanguard start -u <url> -r /any/path/to/repo
```
...
# Workspaces & Resume
./vanguard start -u <url> -r my-repo -w my-audit    # New named workspace
./vanguard start -u <url> -r my-repo -w my-audit    # Resume (same command)
./vanguard workspaces                                 # List all workspaces

# Monitor
./vanguard logs <workspace>            # Tail workflow log
./vanguard status                      # Show running workers
# Temporal Web UI: http://localhost:8233

# Stop
./vanguard stop                        # Preserves workflow data
./vanguard stop --clean                # Full cleanup including volumes (confirms first)

# Image management
./vanguard build [--no-cache]          # Local mode: build worker image
npx @copointai/vanguard uninstall             # npx mode: remove ~/.vanguard/ (confirms first)
...
```
apps/cli/        — @copointai/vanguard (published to npm, bundled with tsdown)
apps/worker/     — @vanguard/worker (private, Temporal worker + pipeline logic)
```

### CLI Package (`apps/cli/`)
Published as `@copointai/vanguard` on npm. Contains only Docker orchestration logic — no Temporal SDK, business logic, or prompts. Bundled with tsdown for single-file ESM output.

- `apps/cli/src/index.ts` — CLI dispatcher (`setup`, `start`, `stop`, `logs`, `workspaces`, `status`, `build`, `uninstall`, `info`)
- `apps/cli/src/mode.ts` — Auto-detection: local mode if `VANGUARD_LOCAL=1` env var is set
- `apps/cli/src/docker.ts` — Compose lifecycle, image pull/build, ephemeral `docker run` worker spawning
- `apps/cli/src/home.ts` — State directory management (`~/.vanguard/` for npx, `./` for local)
...
- `apps/cli/src/config/resolver.ts` — Cascading config (npx only): env vars → `~/.vanguard/config.toml` (parsed with `smol-toml`)
...
- `vanguard` — Node.js entry point (`#!/usr/bin/env node`) that delegates to `apps/cli/dist/index.mjs`
...
- `docker-compose.yml` — Infra only: `vanguard-temporal` (port 7233/8233). Network: `vanguard-net`
- `Dockerfile` — 2-stage build (builder + Kali Linux rolling runtime). Uses pnpm. Entrypoint: `CMD ["node", "apps/worker/dist/temporal/worker.js"]`
...
### Supporting Systems
- **Configuration** — YAML configs in `apps/worker/configs/` with JSON Schema validation (`config-schema.json`). Supports auth settings, MFA/TOTP, and per-app testing parameters. Credential resolution — local mode: env vars → `./.env`; npx mode: env vars → `~/.vanguard/config.toml` (via `shn setup`)
...
**CLI:** `vanguard` (entry point), `apps/cli/src/index.ts` (dispatcher), `apps/cli/src/docker.ts` (orchestration), `apps/cli/src/mode.ts` (auto-detection)
...
- **Worker not processing** — Check `docker ps --filter "name=vanguard-worker-"`
- **Reset state** — `./vanguard stop --clean`
